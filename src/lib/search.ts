...
export function buildPreparedPieces(
	...
	const { gender, hunterType, maxRarity, maxHr, maxElderStars } = settings;
	...
	for (let id = 0; id < armors.length; id++) {
		...
		const elderOk = maxElderStars == null || a.elderStarRequired <= maxElderStars;
		if (!hrOk && !elderOk) continue;
...
/** Validate a leaf (pieces + decoration coverage) and return cheap facts. */
function leafCore(
	ctx: SearchCtx,
	pieces: PreparedPiece[],
	slots: number
	...
	if (deficits.length > 0) {
		...
		const caps = [ctx.baseSlots, ...pieces.map((p) => p.slots)];
		const sizes = [
...
export async function runSearch(
	...
	// No charms in MHFU — search directly with armor pieces only
	progress.phase = 'Searching';
	onProgress?.({ ...progress });
	await searchPieces(ctx, onProgress, signal, progress);
...
async function searchPieces(
	...
	const stack: Frame[] = [];
	stack.push({ depth: 0, pts: ctx.relTrees.map(() => 0), slots: ctx.baseSlots, pieces: [] });
	...
	while (stack.length > 0) {
		...
		if (frame.depth === 5) {
			const core = leafCore(ctx, frame.pieces, frame.slots);
			if (core) {
				ctx.found++;
