<script lang="ts">
	...
	import { aggregateSetMaterials, rarityColor } from '$lib/gameData';
	...
	async function exportImage() {
		...
		try {
			...
			a.download = `mhfu-set-${index + 1}.png`;
			a.href = dataUrl;
			a.click();
		} catch (e) {
			console.error(e);
		...
		<div class="border-t border-zinc-800 px-4 py-3">
					...
					<span class="ml-1 text-zinc-200">
						{#each result.decorations as d (d.name)}
							<span class="mr-2 inline-block">
								{d.name} <span class="text-emerald-400">x{d.count}</span>
							</span>
						{/each}
					</span>
				</div>
				...
				<div class="mt-2 rounded border border-zinc-800 bg-zinc-950/50 p-2 text-xs">
					...
					<div class="mt-1 flex flex-wrap gap-x-4 gap-y-1">
						{#each aggregateSetMaterials( result.pieces.flatMap((p) => p.materials), result.decorations ) as m (m.name)}
							<span class="text-zinc-300">
								{m.name}
								<span class="text-emerald-400">x{m.quantity}</span>
							</span>
						{/each}
					</div>
				</div>
			...
			<div class="mt-3 flex gap-2" data-no-export>
				<button
					type="button"
					onclick={copy}
					class="rounded border border-zinc-700 px-2.5 py-1 text-xs text-zinc-300 hover:border-emerald-500 hover:text-emerald-300"
				>
					Copy set
				</button>
				<button
